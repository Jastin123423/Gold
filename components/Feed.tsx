                ...
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <div className="flex items-center gap-1 flex-wrap">
                            ...
                            {!isOwner && currentUser && 'followers' in author && !isFollowing && onFollow && <button onClick={() => onFollow(author.id)} className="text-[#1877F2] font-bold text-sm hover:underline ml-1">• Follow</button>}
                            {post.feeling && <span className="text-[#B0B3B8] text-[15px]">is feeling {post.feeling}</span>}
                        </div>
                        <div className="flex items-center gap-1 text-[#B0B3B8] text-[13px]"><span>{post.timestamp}</span><span>•</span><i className={`fas ${post.visibility === 'Public' ? 'fa-globe-americas' : 'fa-user-friends'} text-[12px]`}></i></div>
                    </div>
                </div>
